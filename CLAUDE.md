...
**M2 is in progress** (agent integration). M1 is merged to `main`; M2 work is on `m2-cli-agent`. The agent path is a `DaemonSink` (behind `ExportSink`) + a cross-platform Rust **`share-the-mark` CLI** under `cli/`: the extension's background SW POSTs briefs to a localhost daemon (`share-the-mark serve`, `127.0.0.1:8787`, one loopback `host_permission`), which persists them and exposes them to an agent via CLI commands (`share-the-mark pending`/`show`) + a bundled Claude Code skill; the panel surfaces a handoff token (`share-the-mark show <id>`). `share-the-mark request <url>` is the agent-initiated flow (open page → block on a same-origin brief → wake the agent). Daemon lifecycle: explicit `share-the-mark serve`/`start` persist; auto-started daemons get an idle timeout (default 30 min) and self-exit. The shadow-root UI isolates its keyboard events from the host page (`isolateKeyboard`). No MCP. Still deferred: FileSystemSink, native side panel, Firefox e2e.
