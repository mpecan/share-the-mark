...
The **`cli/`** directory is a separate, self-contained Rust crate (binary `share-the-mark`, the M2 daemon) — its own `cargo` build/test/clippy/fmt, gated in CI on a 3-OS matrix; not part of the pnpm/TS toolchain (`cli/target` is gitignored). The bundled agent skill lives at `cli/skill/share-the-mark/SKILL.md` (embedded via `include_str!`).
