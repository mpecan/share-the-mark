...
- `overlay` — imperative, **SVG-only** pointer state machine; re-renders on scroll/resize + observers; caret hit-tests page text for point tools and captures the selection on `mouseup` for highlights. Existing marks are editable: pointerdown on a mark (`data-stm-id`) starts a drag (callout/text move the offset; arrow endpoints/whole-line via handles; highlight start/end handles re-anchor the text range via caret hit-test → fresh `TextAnchor`), and double-clicking a text mark retypes it — all via an `onUpdate` callback and the reducer's `update` action.
